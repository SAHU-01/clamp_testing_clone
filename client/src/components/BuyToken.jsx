...
const BuyTokenModal = ({ cardHeading, token, showModal, onCloseModal }) => {
  ...
  const numBars = token.length === 2 ? 2 : 3;
  const initialStateVal = {
    0: Math.floor(100 / numBars),
    1: Math.floor(100 / numBars),
    2: Math.floor(100 / numBars + 1),
  };
  const [val, setVal] = useState(initialStateVal);
  ...
  const calculateTokenAmount = () => {
    return amount * 0.9998040384084719;
    //return NaN;
  };

  const handleReset = () => {
    setAmount("");
    setVal(initialStateVal);
  };
